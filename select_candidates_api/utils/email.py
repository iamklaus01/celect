...
class Email:
    ...
    async def send_verification_code(self):
        await self.send_mail('Email Verification', 'verify_email')


class GetInTouch:
    def __init__(self, name: str, user_mail: str, message:str, admin_mail: List[EmailStr]):
        self.name = name
        self.sender = 'C-Elect <admin@celect.com>'
        self.mail = user_mail
        self.content = message
        self.admin_mails = admin_mail
    ...
    async def send_mail_to_admin(self):
        await self.send_mail('Get in Touch C-Elect', 'get_in_touch')
...
async def get_in_touch(username:str, user_email:str, content:str):
    try:
        
        query = select(users.c.email).where(users.c.role ==  Role.admin)
        admin_emails = await database.execute(query)

        await GetInTouch(username, user_email, content, admin_emails).send_mail_to_admin()
    except Exception as error:
        print('Error', error)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail='An error occured while sending email')

    return 'Your message has been transmitted to the admin of the platform... They will get back to you as soon as possible !'
